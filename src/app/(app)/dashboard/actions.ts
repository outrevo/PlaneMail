...
export async function getDashboardStats(): Promise<DashboardStats> {
  ...
  try {
    const [subscriberCountResult, newsletterCountResult] = await Promise.all([
      db.select({ count: eq(subscribers.userId, userId) ? 1 : 0 }).from(subscribers).where(eq(subscribers.userId, userId)),
      db.select({ count: eq(newsletters.userId, userId) && eq(newsletters.status, 'sent') ? 1 : 0 }).from(newsletters).where(and(eq(newsletters.userId, userId), eq(newsletters.status, 'sent'))),
    ]);
